...
def forsaleprocess(path):
	with open(path, "r") as f:
		...
		total = [p.find("span", {"class": "converted_price"}).get_text().strip() for p in price] 
		...
		list_price = [p.find("span", {"class": "price"}).get_text().strip() for p in price]
		...
		### SLEEVE CONDITION
		sleeve_cond = map(lambda x: x.find("span", {"class": "item_sleeve_condition"}), cond) 
		sleeve_cond = map(lambda x: x.get_text() if x else "No sleeve listed", sleeve_cond)
		...
		rate_count =  [nr[1].get_text().strip() if len(nr) > 1 else None for nr in name_rate]
		rate_count = [int("".join(filter(str.isdigit, rc))) if rc else None for rc in rate_count]
		
		### AVERAGE RATING
		ave_rate = [s.find_all("strong") for s in seller]
		ave_rate = [ar[1].get_text().strip() if len(ar) > 1 else None for ar in ave_rate]
		...
		biglist = [total, est_exact, media_cond, sleeve_cond, list_price, shipping_price,ships_from, currency, seller_name, rate_count, ave_rate, note] 
		biglist = zip(*biglist)
		return biglist
